...
Deno.serve(async(req)=>{
  ...
  try{
    while(true){
      ...
      if(done)break;
      if(head.byteLength<65544){const take=Math.min(65544-head.byteLength,value.byteLength),nextHead=new Uint8Array(head.byteLength+take);nextHead.set(head);nextHead.set(value.slice(0,take),head.byteLength);head=nextHead}
