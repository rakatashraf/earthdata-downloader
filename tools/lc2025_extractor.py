...
def load_indicator_registry(workbook_path: str) -> pd.DataFrame:
    ...
    if not path.exists():
        ...
        for i,key in enumerate(COMPONENT_KEYS, start=1):
            ...
            records.append({"#":i,"Component":key,"component_key":key,"frontend_unit":unit,"direction":direction,"model_mode":mode})
...
class LupusCortexExtractor:
    ...
    def extract(
        ...
        try:
            ...
            hls = fetch_hls_metrics(lat, lon, hls_start, req, cfg.analysis_area_km, cfg.green_access_m)
        except Exception as exc:
