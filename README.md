...
- Client-side conversion for CSV, JSON/GeoJSON, GeoTIFF, NetCDF classic, HDF5, HE5 and NetCDF4-compatible HDF5 sources
...
A public GitHub Pages site cannot run a persistent Python/Node backend. This implementation therefore performs discovery and supported conversions in the browser. NASA granule formats vary widely. The browser uses GeoTIFF, NetCDF and HDF5/WebAssembly readers and refuses to normalize a dataset when values cannot be aligned to identifiable source latitude/longitude arrays without changing the source grid.
